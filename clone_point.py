...
async def process_voice_clone(request: Request):
    ...
    if not status_id:
        raise HTTPException(status_code=400, detail="Missing X-Status-Id header.")
    if "multipart/form-data" not in content_type:
        raise HTTPException(status_code=415, detail="Expected multipart/form-data request.")
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail="A process is already running. Please try again later."
        )
    ...
    try:
        try:
            ...
            raise HTTPException(status_code=400, detail="Malformed multipart form-data.")
        ...
        try:
                ...
                while True:
                    ...
                    if len(header) < 12:
                        ...
                        if len(header) >= 4 and header[0:4] != b"RIFF":
                            raise HTTPException(status_code=400, detail="Invalid WAV header. Expected RIFF/WAVE format.")
            ...
            try:
                ...
                raise HTTPException(status_code=400, detail="Audio file is corrupted or unreadable.")
            ...
            try:
                ...
                raise HTTPException(status_code=400, detail="Failed to normalize WAV container.")
            try:
                await asyncio.to_thread(shutil.copyfile, safe_path, persisted_safe_wav_path)
            except OSError:
                raise HTTPException(status_code=500, detail="Failed to write sanitized WAV file.")
            ...
            try:
                if not verify_safe_wav(safe_path):
                    raise HTTPException(status_code=400, detail="Sanitized WAV failed strict verification checks.")
                response = await asyncio.to_thread(post_groq_transcription, safe_path)
                response.raise_for_status()
                try:
                    groq_payload = response.json()
                except ValueError:
                    raise HTTPException(status_code=502, detail="Invalid transcription response from Groq.")
                ...
                try:
                    ...
                    raise HTTPException(status_code=500, detail="Failed to write Groq Whisper response file.")
...
if __name__ == "__main__":
    ...
    uvicorn.run("clone_point:app", host="0.0.0.0", port=8001, reload=False)
